...
echo "=========================================="
echo "Setting up Argo CD for hello-world STAGING"
echo "=========================================="
...
echo ""
echo "⚠️  WARNING: You are about to apply secrets to your cluster for essesseff-hello-world-go-template organization and hello-world STAGING"
echo ""
...
if kubectl get secret hello-world-argocd-staging-repo -n argocd &> /dev/null; then
  echo "  ✓ Secret 'hello-world-argocd-staging-repo' exists"
else
  echo "  ✗ Secret 'hello-world-argocd-staging-repo' not found"
  exit 1
fi
if kubectl get secret hello-world-config-staging-repo -n argocd &> /dev/null; then
  echo "  ✓ Secret 'hello-world-config-staging-repo' exists"
else
  echo "  ✗ Secret 'hello-world-config-staging-repo' not found"
  exit 1
fi
...
# Check if argocd/hello-world-staging-application.yaml exists
if [ ! -f "argocd/hello-world-staging-application.yaml" ]; then
  echo "❌ Error: argocd/hello-world-staging-application.yaml not found"
  exit 1
fi

# Apply app-of-apps for STAGING
echo "📝 Applying app-of-apps for STAGING..."
